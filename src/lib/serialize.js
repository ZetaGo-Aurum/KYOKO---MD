...
function createContextInfo(jid, text, title = '', body = '', thumbnail = null) {
    ...
    if (jid && text) {
        ...
        contextInfo.stanzaId = 'KYOKOMD' + Date.now();
    }
    
    if (title || body || thumbnail) {
        contextInfo.externalAdReply = {
            showAdAttribution: true,
            title: title || config.bot?.name || 'KYOKO MD',
            body: body || '',
            mediaType: 1,
...
async function serialize(sock, msg, store = {}) {
    ...
    m.botName = sock.user?.name || config.bot?.name || 'KYOKO MD';
    m.messageId = m.id;
    m.chatId = m.chat;
    m.senderId = m.sender;
        ...
        if (replyVariant === 2) {
            try {
                ...
                contextInfo = {
                    ...contextInfo,
                    externalAdReply: {
                        title: config.bot?.name || 'KYOKO MD',
