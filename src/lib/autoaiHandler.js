...
// Clean AI response from API formatting artifacts
function cleanAIResponse(text) {
    if (!text) return text;
    
    // Remove PERTANYAAN/JAWABAN section headers (common API formatting)
    let cleaned = text
        .replace(/\*?\s*PERTANYAAN\s*\*?:?\s*/gi, '')
        .replace(/\*?\s*JAWABAN\s*\*?:?\s*/gi, '')
        .replace(/\*?\s*Question\s*\*?:?\s*/gi, '')
        .replace(/\*?\s*Answer\s*\*?:?\s*/gi, '')
        .replace(/❓\s*PERTANYAAN:?\s*/gi, '')
        .replace(/💬\s*JAWABAN:?\s*/gi, '')
        .replace(/\?\s*PERTANYAAN/gi, '')
        .replace(/8\)\s*JAWABAN/gi, '');
    
    // Remove duplicate lines - if same content appears twice
...
async function handleAutoAI(m, sock) {
    ...
    try {
        ...
        // Clean AI response - remove PERTANYAAN/JAWABAN headers and duplicates
