...
const pluginConfig = {
    name: 'kyokoconvert',
    alias: ['convertplugin', 'kyokoplugin', 'convertkyoko'],
    category: 'owner',
    description: 'Convert format plugin external ke format KYOKO MD',
    usage: '.kyokoconvert [namafile] [folder]',
    example: '.kyokoconvert sticker sticker',
    isOwner: true,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (!quoted) {
        return m.reply(
            ...
            `└────────────────\n\n` +
            `> \`${m.prefix}kyokoconvert\` - Auto\n` +
            `> \`${m.prefix}kyokoconvert nama\` - Custom`
        )
    }
